...
import { FiCheck, FiX, FiTrash2, FiEdit, FiSearch } from "react-icons/fi";
...
function FacultyNotices() {
    ...
    const fetchNotices = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`https://ns-server.onrender.com/api/notice/teachers-notice`, {
            ...
            toast.error("Failed to fetch teacher notices");
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        if (user?.role === "admin") fetchNotices();
    }, [user]);
            ...
            const filtered = notices.filter(
                (notice) =>
                    ...
                    (notice.createdByUser &&
                        (`${notice.createdByUser.firstName} ${notice.createdByUser.lastName}`)
                            .toLowerCase()
                            .includes(query))
            );
